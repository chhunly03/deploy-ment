    ...
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-orange-50 to-orange-100 p-4 sm:p-6">
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-lg p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <button
            onClick={prevMonth}
            className="px-2 sm:px-3 py-1 bg-orange-100 rounded-lg hover:bg-orange-200 disabled:opacity-50 text-sm sm:text-base"
            disabled={loading}
          >
            ◀
          </button>

          <h1 className="text-lg sm:text-2xl font-bold text-orange-600 text-center">
        ...
        {loading ? (
          ...
          <>
            {/* Weekdays */}
            <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2 text-center font-semibold text-gray-600 text-xs sm:text-base">
            ...
            <div className="grid grid-cols-7 gap-1 sm:gap-2">
              {emptyCellsBefore.map((_, i) => (
                <div key={`empty-before-${i}`} className="h-10 sm:h-16" />
              ))}
              ...
              {emptyCellsAfter.map((_, i) => (
                <div key={`empty-after-${i}`} className="h-10 sm:h-16" />
              ))}
            </div>
          </>
        )}
